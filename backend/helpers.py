...
class helpers():
    ...
    async def chat_with_rag(session_id: str, reqChat: str,language,qa_chain):
        try:
            ...
            question = f"You're a doctor. Answer in concise way. Structure of the answer should be plain text format,  bullet points if needed.if you are not conclusive towards diagnosis ask questions and then give possible outcomes.at a time ask only one question. the subsequent chats will be added in the following prompts. following the diagnosis also give prognosis and treatment. do the conversation as you are the doctor. also consider the current timestamp of the previous chats and their effect. I want the possible diagnosis after maximum 10 questions. before 10 previous conversations conclude and prescribe some medicine. if there is no prev chat do not mention directly that there is not prev chat, instead you can say something like as we have met for the first time and so. if age gender and name is required then only ask with the question.Try to give diagonis as earliest at possible. Try to restrict asking maximum 5 questions. Strictly answer in {language}, keep the medicals terms in english only.do not repeat the prvious chats again and again, just consider them while answering . refer to the prev chat is required.Here is the prev conversation: {previous_conversation} . Here's the question: {reqChat}"
